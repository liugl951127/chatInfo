...
@Slf4j                                                                      // 自动生成 log 字段
/**
 * BotService - 智能客服机器人回复.
 * ----------------------------------------------------------------------------
 * 业务场景: 客户在等待或人工前, 由 Bot 自动回复.
 *
 * 流程:
 *   1) 收到客户消息 → 判断是否机器人模式 (isBot=1)
 *   2) 调 reply(content) 拿回复 (纯本地规则, 不依赖 LLM)
 *   3) MessageService 持久化 + 推送
 *   4) 触发 TransferToHumanEvent 如果包含「人工」关键词
 *
 * 算法 (reply 优先级):
 *   - 精确关键字 (substring) → 匹配预设 FAQ
 *   - 正则 (邮箱/订单号/手机号) → 模板回复
 *   - 兜底 (永不返回 null)
 *   - 同关键词多条候选, 随机返回一条 (增加对话多样性)
 *
 ...
 *     原因: 避免 MessageService<->BotService 的 Spring 循环依赖 (Spring 6 默认禁)
 *     替代方案: 事件总线 / @Lazy / allow-circular-references 都不彻底
 *   - 阶段 2: 可换 OpenAI / Ollama, 改 reply() 即可
 */
